...
test('two nodes in one layer', () => {
    let nodes = [{elements: [{name: "group 1", nodes: [{name: "node 1"}, {name: "node 2"}]}]}];

    addLayerPositionToNode(nodes);

    expect(nodes)
        .toStrictEqual([{elements: [{name: "group 1", nodes: [
            {name: "node 1", key: "0_0", index: 0, relativePosition: 0, layerIndex: 0},
...
test('three nodes in one layer ', () => {
    let nodes = [{elements: [{name: "group 1", nodes: [{name: "node 1"}, {name: "node 2"}, {name: "node 3"}]}]}];
